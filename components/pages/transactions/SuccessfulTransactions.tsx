...
const SuccessfulTransaction: React.FC = () => {
  const [records, setRecords] = useState<SuccessfulTransactionProps[]>([]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: SuccessfulTransactionProps) =>
  ...
  ] as Column<SuccessfulTransactionProps>[];

  const populateData = async () => {
    try {
      setLoading(true);
      const res = await get(
        `/api/admin/successful-txns?page=${currentPage}&page_size=${pageSize}`
      );
      ...
      toast.error('Error fetching successful transactions');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Card className="p-4">
      <h1 className="mb-2 font-semibold">Successful Transactions</h1>
